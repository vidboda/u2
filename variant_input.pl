...
if ($step == 1) { #insert form and possibility to create variants.
	...
	my $name = 'a.nom_prot';
	if ($type ne 'exon') {$name = 'a.nom_ivs'}
	...
	if ($q->param('new_variant') && $q->param('new_variant') =~ /(c\.[>\w\*\-\+\?_]+)/o) {
		...
		my $query = "SELECT nom FROM variant WHERE nom = '$cdna' AND refseq = '$acc_no';";
		my $res = $dbh->selectrow_hashref($query);
		#print $cdna;
		if (!$res->{'nom'}) {
			my $ng_accno;
			...
			if (U2_modules::U2_subs_1::test_mutalyzer() == 1) {
				...
				my $query = "SELECT acc_version, mutalyzer_version, mutalyzer_acc FROM gene where refseq = '$acc_no';";
				my $res = $dbh->selectrow_hashref($query);
					...
					if ($_) {
						foreach (@{$tab_ref}) {
							...
							if ($nom_g =~ /chr\w+:g\.(\d+)_(\d+)[^\d]+/o) {
								...
								my $pos = $1;
								$taille = 1;
								$query = "SELECT numero, type FROM segment WHERE refseq = '$acc_no' AND $pos BETWEEN SYMMETRIC $postgre_start_g AND $postgre_end_g;";
								$res = $dbh->selectrow_hashref($query);
								if ($res) {$num_segment_end = $num_segment = $res->{'numero'};$type_segment_end = $type_segment = $res->{'type'};}
							}
							...
							#for PCDH15 uncomment following
								...
								if ($type_segment eq 'intron') {
									...
									$query = "SELECT nom FROM segment WHERE refseq = '$acc_no' AND numero = '$num_segment';";
									$res = $dbh->selectrow_hashref($query);
									my $nom_segment = $res->{'nom'};
									$query = "SELECT nom FROM segment WHERE refseq = '$acc_no' AND numero = '$num_segment_end';";
									$res = $dbh->selectrow_hashref($query);
								...
								if ($call->result->{'genomicDescription'} =~ /($gid)_\d+\.?\d:(g\..+)/og) {$nom_ng = $2}
                else {$nom_ng = 'NULL'}
								...
								my $insert = "INSERT INTO variant(nom, refseq, nom_g, nom_ng, nom_ivs, nom_prot, type_adn, type_arn, type_prot, classe, type_segment, num_segment, num_segment_end, taille, snp_id, snp_common, commentaire, seq_wt, seq_mt, type_segment_end, creation_date, referee, nom_g_38, defgen_export) VALUES ('$variant', '$acc_no', '$nom_g', '$nom_ng', '$nom_ivs', '$nom_prot', '$type_adn', '$type_arn', '$type_prot', '$classe', '$type_segment', '$num_segment', '$num_segment_end', '$taille', '$snp_id', '$snp_common', 'NULL', '$seq_wt', '$seq_mt', '$type_segment_end', '$date', '".$user->getName()."', '$nom_g_38', '$defgen_export');";
								...
								if ($id ne '') {
									$insert = "INSERT INTO variant2patient (nom_c, num_pat, id_pat, refseq, type_analyse, statut, allele, denovo) VALUES ('$variant', '$number', '$id', '$acc_no', '$technique', '$status', '$allele', '$denovo');\n";
									print $insert;
