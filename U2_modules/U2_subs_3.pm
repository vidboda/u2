...
sub build_roi {
	...
	my $query = "SELECT a.chr, MIN(LEAST(b.start_g, b.end_g)) as min, MAX(GREATEST(b.start_g, b.end_g)) as max FROM gene a, segment b WHERE a.nom[1] = b.nom_gene[1] AND a.ns_gene = 't' AND b.type LIKE '%UTR' GROUP BY a.nom[1], a.chr ORDER BY a.chr, min ASC;";
	my $sth = $dbh->prepare($query);
	my $res = $sth->execute();
