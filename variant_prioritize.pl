...
if ($analysis eq 'Missense') {	
	...
	if ($res2 ne '0E0') {
		...
		while (my $result2 = $sth2->fetchrow_hashref()) {
			...
			my $semaph = 0;
			#if ($result2->{'nom_g_38'} ne '') {
			#	#$result->{'nom_g_38'} =~ /chr([\dXYM]+):g\.(\d+)([ATGC])>([ATGC])/o;
			#	$result2->{'nom_g_38'} =~ /chr($U2_modules::U2_subs_1::CHR_REGEXP):g\.(\d+)([ATGC])>([ATGC])/o;
			...
			#	&dbnsfp2html(\@dbnsfp, $ref38, $alt38, 120, 138, 136, 142, 239, 23, 32, 49, 62, $result2->{'nom_gene'}->[0], $id, $number, $result2->{'nom_gene'}->[1], $result2->{'nom'}, $result2->{'nom_prot'}, $result2->{'statut'});#dbnsfp, ref, alt, onekg, espea, espaa, exac_maf, clinvar, sift, polyphen, fathmm, metalr, gene, id_patient, number_patient, NM_accno, var c., var p., status
